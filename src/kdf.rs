//! Key derivation
//!
//! Multiple secret subkeys can be derived from a single, high-entropy master
//! key.
//!
//! With the master key and a key identifier, a subkey can be deterministically
//! computed. However, given a subkey, an attacker cannot compute the master key
//! nor any other subkeys.
//!
//! The derive_from_key API can derive up to 2^64 keys from a single master key
//! and context, and individual subkeys can have an arbitrary length between 128
//! (16 bytes) and 524,280 bits (65535 bytes).
//!
//! # Examples
//! ```
...
/// Derives a subkey_id-th subkey of length subkey_len bytes using the master
/// key and the context.
